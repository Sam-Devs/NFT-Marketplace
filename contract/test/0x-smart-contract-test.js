...
describe("NFTMarket", function () {
  it("Should create and execute market sales", async function () {
    connector.start().then(async () => {
      ...
      expect(unListedDod.listed).to.equal(false);
      ...
      await nftMarketplace.createMarketItem(1);

      // checking a listed NFT
      const listedDod = await nftMarketplace.oneDod(1);
      expect(listedDod.currentOwner).to.equal(sellerAddress.address);
      expect(listedDod.listed).to.equal(true);
      expect(listedDod.sold).to.equal(false);
      ...
      expect(soldDod.listed).to.equal(false);
      expect(soldDod.sold).to.equal(true);
      expect(soldDod.currentOwner).to.equal(buyerAddress.address);
